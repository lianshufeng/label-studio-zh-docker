...
SKIP_FILE_MARKERS = (".stories.", ".story.", ".spec.", ".test.")

TITLE_RE = re.compile(r"^title:\s*[\"']?(.*?)[\"']?\s*$", re.MULTILINE)
GROUP_RE = re.compile(r"^group:\s*[\"']?(.*?)[\"']?\s*$", re.MULTILINE)
QUOTED_RE = re.compile(r"""(?P<q>["'])(?P<text>[^"'\\\n][^\\\n]{1,320}?)(?P=q)""")
BACKTICK_RE = re.compile(r"`(?P<text>(?:[^`\\]|\\.){1,320})`", re.DOTALL)
CONCAT_QUOTED_RE = re.compile(
    r"""(?P<chunk>(?:(?P<q>["'])(?P<text>[^"'\\\n][^\\\n]{1,220}?)(?P=q)\s*\+\s*)+(?P<q_last>["'])(?P<text_last>[^"'\\\n][^\\\n]{1,220}?)(?P=q_last))"""
)
TAG_TEXT_RE = re.compile(r">([^<>{}][^<>{}]{1,400}?)<", re.DOTALL)
JSX_WHITESPACE_RE = re.compile(r"""\{\s*(?P<q>["'])\s+\1\s*\}""")
ATTR_RE = re.compile(
